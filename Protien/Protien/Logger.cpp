...
string createLogFileName() {
    ...
    sprintf_s(fileName, "Log %02d-%02d-%d %02d-%02d-%02d.log",
...
#else
#ifndef  PTHREAD_MUTEX_ERRORCHECKNP
#define  PTHREAD_MUTEX_ERRORCHECKNP PTHREAD_MUTEX_ERRORCHECK
#endif
    int ret = 0;
    ret = pthread_mutexattr_settype(&m_Attr, PTHREAD_MUTEX_ERRORCHECKNP);
    if (ret != 0) {
...
void Logger::setLogDirectory(string const &logDir) {
    if (!experimental::filesystem::exists(logDir.c_str())) {
        Helpers::create_dir(logDir.c_str());
    }
...
string Logger::getCurrentTime() {
    ...
    char time[100];
    timeinfo = localtime(&rawtime);
    sprintf_s(time, "%02d/%02d/%d %02d:%02d:%02d",
